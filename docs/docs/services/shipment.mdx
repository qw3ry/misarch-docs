...
- [payment/payment/payment-enabled](/docs/docs/services/payment#paymentpaymentpayment-enabled): Used to create new shipments for an order when the order and associated payment is validated successfully.
- [address/user-address/created](/docs/docs/services/address#addressuser-addresscreated): Used to import `UserAddress` from the `Address` bounded context.
- [address/vendor-address/created](/docs/docs/services/address#addressvendor-addresscreated): Used to import `VendorAddress` from the `Address` bounded context.
- [catalog/product-variant-version/created](/docs/docs/services/catalog#catalogproduct-variant-versioncreated): Used to import `ProductVariantVersion` from the `Catalog` bounded context.
- [return/return/created](/docs/docs/services/return#returnreturncreated): Used to create a shipment for a return.
