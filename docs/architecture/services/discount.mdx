...
- [inventory/product-item/reservation-succeeded](/architecture/services/inventory#inventoryproduct-itemreservation-succeeded): Causes a validation of the order to check if the discounts can still be applied.
- [user/user/created](/architecture/services/user#userusercreated): Used to import `User` from the `User` bounded context.
- [catalog/category/created](/architecture/services/catalog#catalogcategorycreated): Used to import `Category` from the `Category` bounded context.
- [catalog/product/created](/architecture/services/catalog#catalogproductcreated): Used to import `Product` from the `Catalog` bounded context.
