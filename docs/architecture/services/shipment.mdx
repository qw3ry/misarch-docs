...
- [payment/payment/payment-enabled](/architecture/services/payment#paymentpaymentpayment-enabled): Used to create new shipments for an order when the order and associated payment is validated successfully.
- [address/user-address/created](/architecture/services/address#addressuser-addresscreated): Used to import `UserAddress` from the `Address` bounded context.
- [address/vendor-address/created](/architecture/services/address#addressvendor-addresscreated): Used to import `VendorAddress` from the `Address` bounded context.
- [catalog/product-variant-version/created](/architecture/services/catalog#catalogproduct-variant-versioncreated): Used to import `ProductVariantVersion` from the `Catalog` bounded context.
- [return/return/created](/architecture/services/return#returnreturncreated): Used to create a shipment for a return.
